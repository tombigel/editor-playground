...
- `Current state`: **Complete, v1 scope** — `src/ai/providers/openRouterAdapter.ts` is a client-direct-to-OpenRouter adapter (`fetch` + SSE streaming, `AbortSignal`-cancellable), no proxy/backend. The `ProviderAdapter` interface is designed for future multi-provider extension but ships OpenRouter-only in v1. The API key lives in `localStorage` only, entered via Settings → AI Assistant, with an inline notice documenting the client-direct key model. `curatedModels.ts` hardcodes a 5-model allowlist across three cost/value tiers rather than the full OpenRouter catalog — `free` (`qwen/qwen3-coder:free`, rate-limited), `low-cost` (`z-ai/glm-5.2`, `moonshotai/kimi-k2-thinking` — cheap models benchmarking close to frontier quality, the default tier), and `good` (`openai/gpt-5.4`, `anthropic/claude-sonnet-5` — top-of-market frontier options). Tier picks were chosen from published OpenRouter pricing/benchmarks across providers, deliberately not defaulted to a single vendor (including this assistant's own model family); the Settings picker groups options by tier.
