...
- Settings has an **AI Assistant** section (`src/panels/settings/sections/AiSettingsSection.tsx`) with a masked OpenRouter API key field, a direct **Get an OpenRouter key** link to `https://openrouter.ai/keys`, a model picker, a conditional custom OpenRouter model-id field, a prompt-caching switch, and a **Check connection** action. The custom model field appears only after choosing **Custom Model**, the final option in the model dropdown.
...
- Prompt caching is opt-in. Its help copy lives in a tooltip beside the setting label. When enabled, the OpenRouter adapter sends the transport-only system prompt as a `cache_control: { type: "ephemeral" }` text block and leaves user/assistant/tool messages unchanged.
...
- The AI panel input footer shows the current model in small muted text. For OpenRouter router modes such as **Free**, **Floor**, and **Auto**, it shows the concrete model id reported by OpenRouter after a response instead of the router id; before the first router response, it says OpenRouter will choose on send.
...
- Assistant messages may show a muted provenance caption (`Answered by ...`) when the responding model id is known, including Auto fallback outcomes.
...
- **Coordination with the AI panel:** Settings and the AI panel (`src/panels/AiPanel.tsx`) are separate React trees — `AiConversationProvider` mounts fresh only while the AI panel is open and is not lifted above Settings in the app shell. Settings therefore does not call `useAiConversation()` directly; instead:
  ...
  - The model picker, custom model field, and prompt-caching switch read/write the persisted conversation state via `conversationStore.tsx`'s own `loadPersistedConversationState`/`persistConversationState` functions (not a re-derived JSON shape), under the same `editor-playground.ai-conversation.v1` key the store already persists to.
  - Because `AiConversationProvider` re-reads localStorage on every mount, a key, model, custom id, or prompt-caching change in Settings takes effect the next time the AI panel is opened (already-open panels are not live-reactive to a Settings change).
