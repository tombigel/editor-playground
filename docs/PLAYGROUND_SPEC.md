...
- Standalone tables are first-class text nodes with `subtype: 'table'`. A table stores exactly one `table` block in `TextDocumentContent.blocks`; rows contain cells, cells contain rich inline nodes only, and optional table `direction` renders as semantic table direction without changing logical storage order.
...
- Standalone tables are also API-first: pure document helpers insert/remove rows and columns, toggle the header row, set per-column alignment, and set table direction before editor UI dispatches those operations.
