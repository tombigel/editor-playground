...
- Standalone tables are first-class text nodes with `subtype: 'table'`. A table stores exactly one `table` block in `TextDocumentContent.blocks`; rows contain cells, cells contain rich inline nodes only, optional table `direction` renders as semantic table direction without changing logical storage order, optional column/row sizing stores CSS lengths on the table block, and table-wide style fields cover table background/border, cell border/padding, and header colors.
...
- Standalone tables are also API-first: pure document helpers insert/remove rows and columns, toggle the header row, set per-column alignment, set table direction, set column widths / row heights, and patch table-wide styles before editor UI dispatches those operations.
