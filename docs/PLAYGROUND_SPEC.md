...
- Table-wide visual defaults live in the inspector `Design` section. While a standalone table is in stage text edit mode, the floating rich text toolbar exposes the active Slate cell context: active row/column sizing with `px`, `em`, and `%` units, active column alignment, row/column insertion and removal relative to the active cell, header and direction toggles, selected cell background/padding, and deterministic border target controls for all/outer/inner/horizontal/vertical/top/right/bottom/left scopes.
