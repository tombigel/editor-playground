...
- The AI panel is viewport-bounded. Long transcripts, warning notices, and draft diff cards share a scrollable content region above the composer, so the input footer and draft approve/reject actions remain reachable even when proposed values or command batches are long.
