...
- The model picker groups its options by cost/value tier — `free` (rate-limited, best for trying the assistant), `low-cost` (cheap, near-frontier quality — the default tier), and `good` (top-of-market frontier, priced accordingly) — with a `SelectSeparator` between tier groups and a tier prefix on each option. The closed trigger shows a compact `tier · model name` value; the open option list additionally shows each model's price/benchmark note. Tier picks span multiple providers and are chosen from published OpenRouter pricing/benchmarks, not defaulted to a single vendor.
