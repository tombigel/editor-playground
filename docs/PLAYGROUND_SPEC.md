...
- Settings has an **AI Assistant** section (`src/panels/settings/sections/AiSettingsSection.tsx`) with a masked OpenRouter API key field, a direct **Get an OpenRouter key** link to `https://openrouter.ai/keys`, a model picker, a custom OpenRouter model-id field, and a prompt-caching switch.
