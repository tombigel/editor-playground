...
- Steps can define a visual anchor with a selector and label. When the target is present, the overlay draws a non-interactive outline-only highlight around the real editor surface using a tour-specific accent, an inset theme-surface ring, and an outer surface halo so it reads separately from editor selection chrome without tinting or dimming large editor areas. Highlight labels use their own solid theme-surface background and must stay readable in light and dark themes. If the target is unavailable, the tour keeps the step visible and explains the intended route.
- Settings-panel story steps should anchor to stable visible shell targets such as `data-showcase-tour-anchor="settings-panel"` when the section content can scroll. They should not highlight tall internal settings sections whose bounding rect can extend outside the visible panel viewport.
- Transfer and validation story steps highlight the left settings nav route (`data-settings-nav="transfer"`) instead of the Import / Export body section, because the body can scroll while the nav item remains a stable wayfinding target.
