...
- Standalone tables are first-class text nodes with `subtype: 'table'`. A table stores exactly one `table` block in `TextDocumentContent.blocks`; rows contain cells, cells contain rich inline nodes only, optional table `direction` renders as semantic table direction without changing logical storage order, and optional column/row sizing stores CSS lengths on the table block.
...
- Standalone tables are also API-first: pure document helpers insert/remove rows and columns, toggle the header row, set per-column alignment, set table direction, and set column widths / row heights before editor UI dispatches those operations.
