...
- Settings has an **AI Assistant** section (`src/panels/settings/sections/AiSettingsSection.tsx`) with a masked OpenRouter API key field, a direct **Get an OpenRouter key** link to `https://openrouter.ai/keys`, a model picker, a custom OpenRouter model-id field, a prompt-caching switch, and a **Check connection** action.
- **Check connection** sends a tiny non-streaming OpenRouter chat-completions request against the resolved selected model id. Its status notice tells the user whether no key is present, the connection has not been checked, the key/model connection is OK, or OpenRouter returned an error such as a rate limit/provider failure.
...
- The model picker defaults to **Free** (`auto:free`). Its first group is **Automatic**: **Free** resolves to the single curated free-tier model with no fallback; **Floor** resolves to the cheapest non-free curated model with `:floor` provider routing; **Auto** tries curated models in ascending price order and retries the next model only for retryable failures such as rate limits/timeouts/provider unavailability. The manual tier groups remain below the Automatic group.
- The custom model field is an always-visible escape hatch. Non-empty trimmed text is persisted directly as `selectedModelId` and sent to OpenRouter verbatim; whitespace-only text is never persisted as a model id.
- Prompt caching is opt-in. When enabled, the OpenRouter adapter sends the transport-only system prompt as a `cache_control: { type: "ephemeral" }` text block and leaves user/assistant/tool messages unchanged.
- The AI panel header includes a settings icon button that opens Settings, so an OpenRouter error in chat has a direct path back to key/model diagnostics.
