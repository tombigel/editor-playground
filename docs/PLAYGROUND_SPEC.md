...
- Page duplication is API-first through `duplicatePage(document, pageId)`: it creates a sibling page with a unique `Copy` display name and slug, copies non-home page settings such as language, visibility, transition, and parent, drops slug aliases to avoid route collisions, clones only page-owned section subtrees with fresh node ids, and leaves shared regions shared.
...
- `Alt` / `Option` during drag duplicates the dragged stage node selection at the resolved preview placement instead of moving the source. The commit uses the duplicate document API, preserves grouped drag relative offsets, supports same-parent and reparent-style drop targets, selects the new duplicate ids, and honors the same parent-expansion result as a normal drag commit.
...
- `Mod + C`: copy the selected stage node payload from stage/editor context; panel UI and rich-text edit mode keep native text copy
- `Mod + V`: paste the editor clipboard payload, or paste external clipboard text/html/link/image data as a new stage node, only from stage/editor context; panel UI keeps native paste
...
- The AI layer never mutates a document directly. Read-only query tools (`getDocumentTree`, `searchNodesByText`, `searchNodesByType`, etc.) execute immediately, since there is nothing to undo. `searchNodesByType` accepts top-level content types (`container`, `text`, `media`) and concrete node subtypes (`image`, `section`, `block`, etc.) so model queries can use the same vocabulary visible in document nodes. Mutation tools (`setRect`, `deleteNode`, `insertSectionTemplate`, and 9 others — the full 12-command v1 surface) are proposed as an `AiDocumentCommand[]` batch, validated, and staged as a **draft**, never applied on proposal.
- `editorApi.applyAiCommands(state, commands)` is the only function that commits a draft, and it is called from exactly one UI approval path in `src/panels/AiPanel.tsx`. The Approve action of the AI draft-review card and the optional safe auto-approve preference both route through that path. Orchestration code that proposes drafts has no access to the dispatcher and cannot apply a batch itself.
...
- The AI panel opens and resizes within viewport-aware width, height, and position bounds. Resizing uses a native-style bottom-right corner hover target with the diagonal resize cursor rather than a visible handle. Long transcripts, warning notices, and draft diff cards share a scrollable content region above the composer, so the input footer and draft approve/reject actions remain reachable even when proposed values or command batches are long.
...
- **Approve** commits the whole batch (see the single-approval-path guarantee above) as one undoable entry, then clears the draft. **Reject** clears the draft and never touches the document.
- **Auto approve** is an opt-in convenience available in the AI panel footer and as a default setting in Settings. When enabled, a newly staged draft is auto-applied only if the batch validates through the same approval path, contains no `deleteNode` command, and was not trimmed by the AI command batch guardrail. Destructive, stale, empty, and overflow-trimmed drafts remain visible for manual approval/rejection and receive an explanatory assistant message.
...
- Settings has an **AI Assistant** section (`src/panels/settings/sections/AiSettingsSection.tsx`) with a masked OpenRouter API key field, a direct **Get an OpenRouter key** link to `https://openrouter.ai/keys`, a model picker, a conditional custom OpenRouter model-id field, prompt-caching and auto-approve switches, a **Check connection** action, and a **Usage** refresh action. The custom model field appears only after choosing **Custom Model**, the final option in the model dropdown.
- **Check connection** sends a tiny non-streaming OpenRouter chat-completions request against the resolved selected model id with a 16-token cap, matching OpenRouter's documented provider minimum. A successful check shows a green check icon beside the button; idle state stays quiet; OpenRouter errors such as rate limits/provider failures show an inline warning message.
- OpenRouter requires an API key even for free-tier models. The UI copy says this directly and frames the key as a free OpenRouter account/no-credit-card requirement, not an optional free-model shortcut.
- The model picker defaults to **Free** (`auto:free`). Its first group is **Automatic** and delegates model choice to OpenRouter: **Free** sends `openrouter/free`, letting OpenRouter randomly choose an available free model that supports the request; **Floor** sends `openrouter/auto` with the Auto Router cost/quality tradeoff set to the lowest-cost preference; **Auto** sends `openrouter/auto` with OpenRouter's default prompt-based model choice. The manual tier groups remain below the Automatic group and send their exact curated OpenRouter model ids.
- The custom model field is a conditional escape hatch shown by the **Custom Model** dropdown option. Non-empty trimmed text is persisted directly as `selectedModelId` and sent to OpenRouter verbatim; whitespace-only text is never persisted as a model id.
- Prompt caching is opt-in. Its help copy lives in a tooltip beside the setting label. When enabled, the OpenRouter adapter sends the transport-only system prompt as a `cache_control: { type: "ephemeral" }` text block and leaves user/assistant/tool messages unchanged. Per-turn system context messages (such as direct-operation context) are never cache-marked.
- Usage refresh calls OpenRouter's official `GET /api/v1/credits` endpoint directly from the browser using the stored key. When the key has the required management permission, Settings shows total purchased credits, used credits, and remaining credits. If OpenRouter rejects the request (for example, a chat-only key without management permission), Settings shows a non-blocking unavailable notice rather than inventing local billing numbers.
- The AI panel header includes a settings icon button that opens Settings directly to the AI Assistant section, so an OpenRouter error in chat has a direct path back to key/model diagnostics.
- The AI panel includes a clear conversation action that removes the visible transcript and any pending draft while preserving model selection, prompt-caching preference, and provider key settings.
...
- **Coordination with the AI panel:** Settings and the AI panel (`src/panels/AiPanel.tsx`) are separate React trees — `AiConversationProvider` mounts fresh only while the AI panel is open and is not lifted above Settings in the app shell. Settings therefore does not call `useAiConversation()` directly; instead:
  ...
  - The model picker, custom model field, prompt-caching switch, and auto-approve switch read/write the persisted conversation state via `conversationStore.tsx`'s own `loadPersistedConversationState`/`persistConversationState` functions (not a re-derived JSON shape), under the same `editor-playground.ai-conversation.v1` key the store already persists to.
  - `persistConversationState` emits a same-window change event after Settings writes. An already-open AI panel listens through `AiConversationProvider` and refreshes its selected model/custom id, prompt-caching, and auto-approve state immediately; remounting the panel is not required.
...
- Standalone tables are first-class text nodes with `subtype: 'table'`. A table stores exactly one `table` block in `TextDocumentContent.blocks`; rows contain cells, cells contain rich inline nodes only, optional table `direction` renders as semantic table direction without changing logical storage order, optional column/row sizing stores CSS lengths on the table block, table-wide style fields cover table background/border, default cell border/padding, and header colors, and individual table cells may store Slate-compatible JSON style fields for background, padding, and border edges.
- Table-wide visual defaults live in the normal inspector `Design` section. Its outer border and shadow use the shared design control groups (including border radius and shadow spread), while cell padding, cell border, and header colors remain table-specific. Table and cell border widths use CSS-valid `px`/`em` controls, while cell padding supports `px`/`em`/`%`; these numeric fields show their effective values even when the model inherits a table default. While a standalone table is in stage text edit mode, a compact second row in the floating rich text toolbar exposes the active Slate cell context: active row/column sizing with `px`, `em`, and `%` units, active column alignment, selected cell background/padding, header and direction toggles, and explicit Row/Column action menus for insertion on either side or deletion. Authored column widths immediately drive the editable table's fixed layout, and a table with every column sized can shrink to those authored widths; authored row heights constrain both the semantic row and its cells. Unit changes preserve rendered size by converting through pixels using the active table/cell dimensions and computed font size. Unsized rows and columns display `auto` as a keyword without a unit; choosing a numeric unit measures the rendered row or column and immediately stores that value. Sizing fields keep their own focus and drafts while committing root-only table size changes in place, so Enter, blur, and unit changes do not rebuild the active Slate cell subtree.
- The table toolbar exposes one Borders popover instead of nine permanent edge buttons. Its `Cell` / `Row` / `Column` / `Table` target is resolved from the active Slate cell, and its all/outer/inner/horizontal/vertical/top/right/bottom/left edge commands expand through the existing Slate-backed table selection APIs. Edge presets that cannot affect the chosen target rectangle are disabled.
...
Container, section, header, and footer wrappers support a gradient background layered over the base color ("color behind + gradient on top"), gated behind an on/off switch (single gradient today).
...
- `Monochrome` forces paintable shape fills to a single color via a `sp-svg-mono` class rule (`fill: currentColor`) with the color carried inline on the root element; authored `fill` and inline `fill` styles are overridden, while explicit no-paint cases (`fill="none"`, `fill="transparent"`, `color="none"`, `color="transparent"`, or `fill-opacity="0"`, including equivalent inline styles) stay untouched. Opacity rides on the color's own alpha channel (picker supports alpha), there is no separate opacity field. Enabling monochrome or stroke seeds real color/width values into the model so controls always reflect stored data.
- `Stroke` applies global stroke styling via a `sp-svg-stroke` class rule and CSS custom properties. Stroke width and dash offset accept `px`/`em` lengths (unitless input is normalized to `px`); cap changes seed a matching default join (`round` → `round`, `square` → `bevel`, otherwise `miter`). Authors can set dash pattern through six alternating dash/gap slots, choose whether the stroke should scale with the shape (`vector-effect: non-scaling-stroke` when off), set stroke color, and set paint order. Dash offset remains supported by the model and renderer but is not exposed in the inspector until stroke animation controls need it.
- Root SVG overflow is independent of stroke: `hidden` is the default and emits no style, while `visible` emits `overflow: visible !important` on the `<svg>`.
...
- The settings panel is centered, scrollable, and uses compact sticky left anchor links for `UI`, `Pages`, `Defaults`, `Fonts`, `AI Assistant`, `Import / Export`, `Advanced`, and `Shortcuts`.
...
- The showcase navigation stays aligned to the rendered demos, with `Logo Assets` promoted as the first top-level section before `Design Tokens`, adjacent `Searchable Select` and `Searchable Multi Select` entries immediately after `Dropdown (Select)`, the hidden selection chrome variant, and AI assistant review UI listed under `Panels`.
...
- Focused `Design` mode combines `Text style` and `Design` into one floating card for text, link, and button nodes; wrappers, images, and tables keep their subtype-specific `Design` card. Focused table `Content` and `Design` modes route to the same table-specific sections used by the normal inspector.
...
- Standalone tables are also API-first: pure document helpers insert/remove rows and columns, toggle the header row, set per-column alignment, set table direction, set column widths / row heights, patch table-wide styles, patch Slate cell styles, and expand selection border scopes into per-cell edge styles before editor UI dispatches those operations.
...
- The first-phase tour map was fixed by the phase 1 research brief (now `archive/SHOWCASE_TOUR_PHASE_1.md`) and is implemented as typed config in `src/app/showcaseTour/showcaseTourConfig.ts`, which is the current source of truth.
- The Start Here topic includes an AI Assistant step that opens the real floating AI panel. The step must describe the interface as a limited MVP with a minimal harness and command set, and must state that it currently supports only the user's own OpenRouter subscription.
...
- Steps can define a visual anchor with a selector and label. When the target is present, the overlay draws a non-interactive outline-only highlight around the real editor surface using a tour-specific accent, an inset theme-surface ring, and an outer surface halo so it reads separately from editor selection chrome without tinting or dimming large editor areas. Highlight movement uses a short CSS-only transition. Highlight labels use their own solid theme-surface background and must stay readable in light and dark themes. If the target is unavailable, the tour keeps the step visible and explains the intended route.
...
- Single-block rich-to-simple split conversion still uses the flattening conversion path.
...
- The snap toggle includes tooltip guidance for `Cmd/Ctrl` drag bypass and the `Shift + G` shortcut.
...
- standalone table edit mode uses the same floating toolbar shell for contextual cell controls; the controls edit Slate-backed table block and cell fields only, and do not introduce stage rails, drag handles, or transient renderer-only table styling
...
- Browser-native undo/redo, selection, and caret navigation should keep falling through to the editor/browser text surface instead of being intercepted by app-global shortcuts. HTML clipboard paste in rich/block/list edit mode is intercepted by the rich-text surface so styled clipboard content can be converted into compatible Slate content at the active selection; plain text paste and code editing remain plain text.
