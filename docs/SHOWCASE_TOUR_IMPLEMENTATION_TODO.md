...
- Tour overlay surfaces opt into the shared editor outside-click exemption contract so tour clicks/drags are neutral to unrelated panels and popovers that use either `useClickOutside` or the app-level floating-panel dismiss listener.
...
- Outside-click exemption follow-up:
  ...
  - `pnpm run build`
  - Notes: sandboxed smoke/build again failed only at localhost server binding (`EPERM` on `127.0.0.1:4174`). Escalated reruns passed.
- App-level floating-panel exemption follow-up:
  - `pnpm run lint`
  - `pnpm run typecheck`
  - `pnpm run test:e2e:smoke`
  - `pnpm run build`
