...
3. **Only explicit user approval commits a draft.** `editorApi.applyAiCommands` (see `docs/API_EDITOR.md`) is the single function that turns an approved draft into an actual document mutation. It is called once, at the moment the user approves, and the entire batch lands as **one undoable history entry** — never a change that appears before approval, and never a series of separate undo steps for a single approved batch.
