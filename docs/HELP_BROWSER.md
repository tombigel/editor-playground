...
5. Restart `pnpm run dev` (or run any build/test command). The sync plugin regenerates the help-doc manifest and copies the markdown files and docs assets into `public/assets/help-docs/` automatically. To trigger it without a server restart, run `pnpm run sync:public`.
