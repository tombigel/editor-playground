...
- `setTableColumnWidthDoc(document, nodeId, columnIndex, width)`
- `setTableRowHeightDoc(document, nodeId, rowIndex, height)`
- `setTableStyleDoc(document, nodeId, patch)`
- `setTableCellStyleDoc(document, nodeId, rowIndex, columnIndex, patch)`
- `setTableSelectionStyleDoc(document, nodeId, selection, patch)`
- `setTableSelectionBorderDoc(document, nodeId, selection, scope, patch)`

They no-op for missing or non-table nodes, preserve at least one row and one column, preserve table direction, sizing, table-wide style metadata, and Slate cell styles, and normalize the table after mutation. Selection border scopes expand into deterministic per-cell edge style fields instead of storing editor selection state.
