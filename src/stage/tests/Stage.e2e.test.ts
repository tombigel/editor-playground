...
const runRichTextE2E = process.env.RICHTEXT_E2E === "1";
const stageIt = runRichTextE2E ? it.skip : it;
const richTextIt = runRichTextE2E ? it : it.skip;
// Keep known-unstable rich authoring cases visible while the rich E2E lane is isolated.
const richTextTodo = it.skip;
...
describe("stage/Stage e2e", () => {
	...
	async function expectSingleSelectionOverlayToMatchNode(nodeId: string) {
		...
		expect(
			Math.abs((overlayBox?.x ?? 0) - ((nodeBox?.x ?? 0) - 2)),
		).toBeLessThan(1);
		expect(
			Math.abs((overlayBox?.y ?? 0) - ((nodeBox?.y ?? 0) - 2)),
		).toBeLessThan(1);
		expect(
	...
	function expectStandaloneListNode(node: TextNode) {
		...
		expect(node.content.blocks).toHaveLength(1);
		expect(
			node.content.blocks[0].type === "ul" ||
				node.content.blocks[0].type === "ol",
		).toBe(true);
	}
	...
	async function dragSelectRichTextRange(
		startNodeIndex: number,
		startOffset: number,
		endNodeIndex: number,
		endOffset: number,
		...
		const points = await page.evaluate(
			({ startNodeIndex, startOffset, endNodeIndex, endOffset }) => {
				...
				const resolvePoint = (
					...
					if (!textNode) {
						throw new Error(`Expected text node ${nodeIndex}`);
					}
					...
					if (!rect) {
						throw new Error(`Expected text node ${nodeIndex} to be visible`);
					}

					return {
						x: edge === "start" ? rect.left + 1 : rect.right + 8,
						y: rect.top + rect.height / 2,
					};
				};

				return {
					start: resolvePoint(startNodeIndex, startOffset, "start"),
					end: resolvePoint(endNodeIndex, endOffset, "end"),
				};
			},
			{ startNodeIndex, startOffset, endNodeIndex, endOffset },
		);
	...
	async function selectSingleRichTextRangeWithKeyboard(
		startOffset: number,
		length: number,
	) {
		await page
	...
	async function selectAllRichText() {
		await page
			.locator('[data-stage-rich-edit-box="true"] [contenteditable="true"]')
			.first()
			.click();
	...
	async function saveCodeEdit() {
		await page
			.locator('[data-stage-code-edit-textarea="true"]')
			.first()
			.click();
		await page.keyboard.press("ControlOrMeta+Enter");
		await page.waitForSelector('[data-stage-code-toolbar="true"]', {
	...
	stageIt(
		"selects and drags a stage text node",
		async () => {
			await openEditor();
			const title = page
				.locator(".stage-leaf", {
	...
	stageIt(
		"collapses a multi-selection back to a single node on a plain second click and shows the group outline only while grouped",
		async () => {
			await openEditor();
			const title = page
				.locator(".stage-leaf", {
	...
	stageIt(
		"keeps the single-selection overlay aligned when selected layout shifts without a new selection event",
		async () => {
	...
	stageIt(
		"resizes a selected image node from the south-east handle",
		async () => {
			await openEditor();
			const image = page.locator(".stage-leaf.subtype-image").first();
	...
	stageIt(
		"preserves image aspect ratio when resizing from a corner with Shift",
		async () => {
			await openEditor();
			const image = page.locator(".stage-leaf.subtype-image").first();
	...
	richTextIt(
		"keeps rich edit mode active when the floating toolbar is clicked",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument();
			await openEditor({
				document,
			...
			expect(
				await page.locator('[data-stage-rich-toolbar="true"]').count(),
			).toBe(1);
			const persistedState = await readPersistedState();
	...
	richTextTodo(
		"allows mouse text selection inside the rich text edit box",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument();
			await openEditor({
				document,
			...
			await dragSelectRichTextRange(0, 0, 0, 8);
	...
	richTextIt(
		"drags the rich toolbar and remembers its position for the current browser session",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument();
			await openEditor({
				document,
	...
	richTextIt(
		"commits rich text changes on outside click",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument();
			await openEditor({
				document,
			...
			await page
				...
				.click();
			await page.keyboard.press("ControlOrMeta+A");
			await page.keyboard.type("Edit me on stage updated");
	...
	richTextIt(
		"discards rich text changes on Escape",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument();
			await openEditor({
				document,
	...
	richTextIt(
		"applies anchor links through the shared link picker and preserves edit mode while the popover is open",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument();
			await openEditor({
				document,
	...
	richTextIt(
		"preserves a partial inline selection when link-popover dropdowns are used",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument(
			...
			await enterRichEditMode(richTextId);
			await selectSingleRichTextRangeWithKeyboard(9, 3);
	...
	richTextIt(
		"keeps a visible retained highlight when focus moves into the toolbar",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument(
				[{ type: "paragraph", children: [{ text: "text one two three" }] }],
	...
	richTextIt(
		"shows structure-specific toolbar dropdowns only for the active mode",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument();
			await openEditor({
				document,
			...
			expect(
				await page.locator('[aria-label="Ordered list marker"]').count(),
			).toBe(0);
			expect(
				await page.locator('[aria-label="Unordered list marker"]').count(),
			).toBe(0);
			expect(await page.locator('[aria-label="Code language"]').count()).toBe(
				0,
			);
			...
			const codeButtonBox = await toolbar
				.getByRole("button", { name: "Use code block" })
				.boundingBox();
			const codeLanguageBox = await toolbar
				.locator('[aria-label="Code language"]')
				.boundingBox();
			expect(codeButtonBox).not.toBeNull();
			expect(codeLanguageBox).not.toBeNull();
			expect(
				(codeLanguageBox?.x ?? 0) - (codeButtonBox?.x ?? 0),
			).toBeGreaterThan(0);
	...
	richTextIt(
		"unwinds nested rich edit layers one step at a time on outside click",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument();
			await openEditor({
				document,
	...
	richTextIt(
		"unwinds nested rich edit layers one step at a time on Escape",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument();
			await openEditor({
				document,
	...
	richTextIt(
		"converts the containing block when a partial inline selection changes block type",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument(
				[{ type: "paragraph", children: [{ text: "text one two three" }] }],
	...
	richTextTodo(
		"preserves touched block boundaries when block type changes across a multi-block selection",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument(
				[
			...
			await expect
				.poll(async () => {
					const persistedState = await readPersistedState();
					return persistedState.document.nodes[richTextId].content.blocks;
				})
				.toEqual([
	...
	richTextIt(
		"treats block boundaries as hard line breaks when converting a multi-block selection to a list",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument(
				[
					{ type: "paragraph", children: [{ text: "one two" }] },
	...
	richTextIt(
		"applies ordered-list marker changes from the dropdown without losing the authored selection",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument(
	...
	richTextIt(
		"persists line height and block spacing changes from the floating toolbar",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument();
			await openEditor({
				document,
			...
			expect(persistedState.document.nodes[richTextId].content.blockGap).toBe(
				24,
			);
	...
	richTextTodo(
		"applies font size changes immediately while the toolbar input keeps focus",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument();
			await openEditor({
				document,
			...
			await expect
				.poll(async () =>
					page
						.locator(
							'[data-stage-rich-edit-box="true"] [style*="font-size: 32px"]',
						)
						.count(),
				)
				.toBeGreaterThan(0);
	...
	richTextIt(
		"unwinds font-size suggestions before closing the rich toolbar on outside click",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument();
			await openEditor({
				document,
	...
	richTextIt(
		"keeps text and highlight color swatches the same size in the rich toolbar",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument();
			await openEditor({
				document,
			...
			const swatchRects = await page.evaluate(() => {
				function readPickerRect(label: string) {
					...
					const trigger =
						host.shadowRoot.querySelector<HTMLElement>(".trigger");
	...
	richTextIt(
		"converts touched blocks into rich code blocks and applies the selected code language",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument(
				[
			...
			await enterRichEditMode(richTextId);
			await selectRichTextRange(0, 1, 0, "const one = 1;".length);
			await page
				.locator('[data-stage-rich-toolbar="true"]')
				.first()
				.getByRole("button", { name: "Use code block" })
	...
	richTextIt(
		"keeps the standalone code toolbar draggable and its language menu selectable",
		async () => {
			...
			await languageMenu
				.locator('[data-ui="select-item"]', { hasText: "Markdown" })
				.click();
			await page.waitForFunction(() => {
	...
	richTextIt(
		"grows the on-stage rich editor as new blocks are added",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument(
				undefined,
				{
			...
			await editable.click();
			await page.keyboard.press("ControlOrMeta+A");
			await page.keyboard.type("Edit me on stage");
	...
	richTextIt(
		"enters standalone block edit mode on second click and commits typed changes on outside click",
		async () => {
			const { document, blockTextId } =
				createBlockTextEditE2EDocument("Block edit");
	...
	richTextIt(
		"commits standalone block edits with Cmd/Ctrl+Enter and discards them with Escape",
		async () => {
	...
	richTextIt(
		"keeps Enter and Shift+Enter as soft breaks inside one standalone text block",
		async () => {
	...
	richTextIt(
		"preserves standalone block inline marks, font size, text color, and highlight through commit",
		async () => {
			const { document, blockTextId } =
				createBlockTextEditE2EDocument("Styled block");
			(document.nodes[blockTextId] as TextNode).content =
				createTextDocumentContent([
					{
						type: "paragraph",
						direction: "ltr",
	...
	richTextIt(
		"persists two inline links in one standalone text block",
		async () => {
			const { document, blockTextId } =
	...
	richTextIt(
		"promotes whole-node block links to inline links and clears node.link on commit",
		async () => {
	...
	stageIt(
		"pushes following root sections when a top-level section is resized from the bottom knob",
		async () => {
	...
	stageIt(
		"reparents a leaf into another container",
		async () => {
	...
	stageIt(
		"falls back to moving within the current parent when the hovered drop wrapper is invalid",
		async () => {
			const { document, ids } = createE2EDocument();
			await openEditor({ document });
	...
	stageIt(
		"locks drag movement to one axis while Shift is held",
		async () => {
			const { document, ids } = createE2EDocument();
	...
	stageIt(
		"nudges the selected element by 1px and 10px with arrow keys",
		async () => {
			const { document, ids } = createE2EDocument();
			await openEditor({ document, snapEnabled: false });
			...
			await page.waitForFunction(
				({ storageKey, nodeId }) => {
					...
					return (
						state?.document?.nodes?.[nodeId]?.rect?.x?.base?.raw === "131px"
					);
				},
				{
					storageKey: STORAGE_KEY,
			...
			const state = await readPersistedState();
			expect(state.document.nodes[ids.axisLeafId].rect.x.base.raw).toBe(
				"131px",
			);
	...
	stageIt(
		"respects the snap toggle shortcut state during drag",
		async () => {
			const { document, ids } = createE2EDocument();
			await openEditor({ document, snapEnabled: true });
	...
	stageIt(
		"suppresses native browser drag behavior inside the stage",
		async () => {
			await openEditor({ document: createDragSuppressionE2EDocument() });

			const result = await page.evaluate(() => {
				const stage =
					window.document.querySelector<HTMLElement>(".stage-shell");
	...
	stageIt(
		"drags same-parent multi-selection as a group",
		async () => {
			const { document, ids } = createE2EDocument();
			await openEditor({ document, snapEnabled: false });
	...
	richTextIt(
		...
		async () => {
			...
			(document.nodes[listTextId] as TextNode).content =
				createTextDocumentContent([
					{
						type: "ul",
						markerStyle: "square",
	...
	richTextIt(
		"handles standalone list Tab, Shift+Tab, and Backspace at item start",
		async () => {
			const { document, listTextId } = createListTextEditE2EDocument();
			(document.nodes[listTextId] as TextNode).content =
				createTextDocumentContent([
	...
	richTextIt(
		"keeps inline standalone list content intact through stage edit commit",
		async () => {
			const { document, listTextId } = createListTextEditE2EDocument();
			(document.nodes[listTextId] as TextNode).content =
				createTextDocumentContent([
	...
	richTextTodo(
		"uses the same Enter behavior inside rich text list blocks",
		async () => {
			const { document, richTextId } = createRichTextEditE2EDocument([
				{
					type: "ul",
	...
	stageIt(
		"drags only the parent when a parent and child are both selected",
		async () => {
			const { document, ids } = createE2EDocument();
			await openEditor({ document, snapEnabled: false });
			...
			await page.waitForFunction(
				({ storageKey, nodeId, beforeX }) => {
					...
					return (
						state?.document?.nodes?.[nodeId]?.rect?.x?.base?.raw !== beforeX
					);
				},
				{
					storageKey: STORAGE_KEY,
	...
	stageIt(
		"falls back to dragging only the anchor for mixed-parent multi-selection",
		async () => {
			const { document, ids } = createE2EDocument();
			await openEditor({ document, snapEnabled: false });
			...
			expect(
				after.document.nodes[ids.otherContainerLeafId].rect.x.base.raw,
			).toBe(beforeOtherX);
...
function createListTextEditE2EDocument(): {
	document: DocumentModel;
	listTextId: string;
	sectionId: string;
} {
...
function createCodeTextEditE2EDocument(): {
	document: DocumentModel;
	codeTextId: string;
	sectionId: string;
} {
