		...
		>
			<span className="flex min-w-0 items-center gap-2">
				<span className="flex h-3.5 w-3.5 shrink-0 items-center justify-center">
					{selected ? (
						<Check className="editor-text-muted h-3.5 w-3.5" />
					) : null}
				</span>
...
export const FontPickerPopover = memo(function FontPickerPopover({
	...
	const [internalRecentFamilies, setInternalRecentFamilies] = useState<
		string[]
	>(() => (managedRecent ? readRecentFontFamilies() : []));
	const recentFamilyNamesResolved =
		externalRecentFamilies ?? internalRecentFamilies;
	const setRecentFamilyNames = managedRecent
		? setInternalRecentFamilies
		: (_updater: (current: string[]) => string[]) => {};
	const orderedFamilies = useMemo(
		() => orderFontFamiliesForPicker(families, recentFamilyNamesResolved),
		[families, recentFamilyNamesResolved],
	);
	const [frozenFamilies, setFrozenFamilies] =
		useState<OrderedFontFamilyGroups | null>(null);
	const visibleFamilies = open
		? (frozenFamilies ?? orderedFamilies)
		: orderedFamilies;
	...
	const activeFamily =
		activeFamilyValue === systemOptionValue
			? undefined
			: families.find((family) => family.family === activeFamilyValue);
	const activeWeightOptions = listFontWeightOptions(activeFamily, weightValue);
	const activeWeightOption =
		activeWeightOptions.find((option) => option.value === weightValue) ??
		activeWeightOptions[0];
	...
	const triggerLabel = mixedFamily
		? "Mixed"
		: familyValue === systemOptionValue
			? systemLabel
			: familyValue;
	const triggerStyle = mixedFamily
		? undefined
		: familyValue !== systemOptionValue
			? mixedWeight
	...
	useEffect(() => {
		...
		const existing = window.document.getElementById(
			previewLinkId,
	...
	function rememberRecentFontFamily(nextValue: string) {
		...
		const computeNext = (current: string[]) =>
			[
				nextValue,
				...current.filter((familyName) => familyName !== nextValue),
			].slice(0, RECENT_FONT_FAMILIES_LIMIT);
	...
	function focusWeightOptionAt(index: number) {
		const clampedIndex = Math.max(
			0,
			Math.min(activeWeightOptions.length - 1, index),
		);
	...
	function handleFamilySelect(nextValue: string) {
		...
		if (nextValue !== systemOptionValue) {
			...
			const resolvedWeight = resolveNearestSupportedFontWeight(
				weightValue,
				nextFamily,
			);
					...
					<div className="editor-scrollbar max-h-[18rem] overflow-auto p-1">
						...
						{visibleFamilies.recent.map((family, index) => (
							<FontPickerRow
								...
								focused={
									activePane === "family" && activeFamilyValue === family.family
								}
								ref={(node) => {
						...
						{visibleFamilies.recent.length > 0 &&
						visibleFamilies.byLanguage.length > 0 ? (
						...
						{visibleFamilies.byLanguage.map((family, index) => (
							<FontPickerRow
								...
								ref={(node) => {
									familyRefs.current[
										visibleFamilies.recent.length + index + 1
									] = node;
								}}
								style={{ fontFamily: buildFontFamilyStack(family.family) }}
								onMouseEnter={() => setActiveFamilyValue(family.family)}
						...
						{activeWeightOptions.map((option, index) => (
							<button
								...
								style={
									activeFamily
										? {
												fontFamily: buildFontFamilyStack(activeFamily.family),
