...
type LayersPanelContentProps = Omit<
  LayersPanelProps,
  'open' | 'position' | 'onPositionChange' | 'panelRef' | 'onOpenChange' | 'onClose'
>;
...
export function LayersPanelContent({
  ...
  const draggedRow = dragState ? rows.find((row) => row.id === dragState.nodeId) ?? null : null;
  ...
  useEffect(() => {
    ...
    function handlePointerMove(event: PointerEvent) {
      ...
      const shouldActivate = currentDrag.active || Math.hypot(deltaX, deltaY) >= 5;
      if (!shouldActivate) {
        return;
      }
      ...
      const nextTarget = resolveLayersDropTarget(document, currentDrag.nodeId, hoveredRow.id, position);
  ...
  function handleRowPointerDown(event: ReactPointerEvent<HTMLDivElement>, row: LayersTreeRow) {
  ...
  function handleRowClick(event: ReactMouseEvent<HTMLDivElement>, rowId: NodeId) {
...
function LayersTreeRowItem({
  ...
  onPointerDown: (event: ReactPointerEvent<HTMLDivElement>, row: LayersTreeRow) => void;
  ...
  const DisclosureIcon: LucideIcon = row.isExpanded ? ChevronDown : ChevronRight;

  return (
    <>
      {row.dividerBefore ? <div aria-hidden="true" className="editor-layers-divider" /> : null}
      <div
      ...
      >
        {row.hasChildren ? (
          <Button
            ...
            aria-label={row.isExpanded ? `Collapse ${row.displayName}` : `Expand ${row.displayName}`}
            onClick={(event) => {
            ...
            {editing ? (
              <span className="block min-w-0" data-layers-title-editor="true">
                <LayersRowTitleEditor
                  ...
                  onCommit={(value) => {
                    onRenameNode(row.id, resolveSidebarTitleCommit(value, row.node.role));
                    onStopEditing();
                  }}
        ...
        <div className="editor-layers-row-actions flex shrink-0 items-center gap-1">
          {invalidDrop ? (
            ...
            >
              <span className="editor-layers-invalid-drop-indicator" aria-hidden="true">
          ...
          >
            <Button
              ...
              data-layers-control="true"
              aria-label={row.node.visible ? `Hide ${row.displayName}` : `Show ${row.displayName}`}
              onClick={(event) => {
                event.stopPropagation();
...
function clampToViewport(value: number, size: number, viewportSize: number) {
  return Math.round(
    Math.max(PANEL_VIEWPORT_MARGIN_PX, Math.min(viewportSize - size - PANEL_VIEWPORT_MARGIN_PX, value)),
  );
}
