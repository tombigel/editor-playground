...
function createConversationStub(
	overrides: Partial<AiConversationApi> = {},
): AiConversationApi {
...
describe("panels/AiPanel", () => {
	...
	it("system prompt requires concise human-readable answers with a next action", () => {
		...
		expect(AI_SYSTEM_PROMPT).toContain(
			"Do not paste the tool result back to the user",
		);
	});
	...
	it("handles help routes locally by opening the AI guide and recording a transcript", () => {
		...
		expect(conversation.messages.at(-1)?.content).toContain(
	...
	it("keeps destructive and overflowed drafts on the manual approval path", () => {
		expect(
			getAutoApproveDraftDecision([{ type: "deleteNode", nodeId: "node-1" }], {
				draftOverflowed: false,
			}),
	...
	it("handles undo and redo routes locally through app history callbacks", () => {
		...
		expect(conversation.messages.at(-3)?.content).toContain(
			"Undid the last change",
		);
		expect(conversation.messages.at(-1)?.content).toContain(
			"Redid the last undone change",
		);
	});
	...
	it("records the concrete model reported by the provider stream", async () => {
		...
		expect(outcome.resolvedModelId).toBe(
...
// `useAiChat`'s `sendMessage`/`cancel` end-to-end behavior (send loop, tool
// routing, draft staging, error handling, cancellation) has its own dedicated
// suite in `useAiChat.test.tsx`, since it doesn't exercise the `AiPanel`
// component itself.
