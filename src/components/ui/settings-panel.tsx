...
export function LabeledControlRow({
	...
	const labelClass = cn(
		"min-w-0 flex-1 whitespace-nowrap text-[11px] font-medium",
		labelClassName,
	);
	...
	return (
		<div
			...
			data-ui="labeled-control-row"
		>
			{typeof label === "string" ? (
				<Label className={labelClass}>{label}</Label>
			) : (
				<div className={cn("editor-text-muted", labelClass)}>{label}</div>
			)}
			<div
				className={cn(
...
const NOTICE_TONE_CLASSES = {
	...
	message:
		"editor-bg-subtle editor-border-subtle editor-text-muted rounded-lg border",
	...
	warning:
		"editor-warning-surface editor-border-subtle editor-warning-text rounded-lg border",
} as const;
...
export function NoticeSurface({
	...
	return (
		<div
			className={cn(
				"flex items-start gap-2 px-3 py-2 text-xs leading-5",
		...
		>
			{compact ? (
				<div className="min-w-0">
					<div className={cn("text-sm font-medium", titleClassName)}>
						{title}
					</div>
					{description ? (
						<div
							className={cn(
								"settings-nav-link-copy mt-0.5 text-xs leading-5",
								descriptionClassName,
							)}
						>
							{description}
		...
		<div className="editor-border-subtle flex items-start justify-between gap-4 border-t py-4 first:border-t-0">
			<div className="flex min-w-0 gap-3">
				...
				<div className="min-w-0">
					...
					<div className="editor-text-muted mt-1 text-sm">{description}</div>
					{note ? (
						<div className="editor-text-muted mt-1 text-xs">{note}</div>
					) : null}
				</div>
			</div>
			<Switch
...
export function ActionRow({
	...
	return (
		<div className="editor-border-subtle flex flex-col gap-3 border-t py-3 first:border-t-0 sm:grid sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start sm:gap-6">
			...
			<div className={`shrink-0 sm:justify-self-end ${actionsClassName ?? ""}`}>
				{actions}
			</div>
		</div>
	);
	...
	return (
		...
		>
			<Button
				...
				variant="ghost"
				size="icon"
				className="editor-text-muted inline-flex h-4 w-4 rounded-sm p-0 hover:text-[color:var(--editor-utility-text-strong)] focus-visible:outline-offset-1"
