...
export function composeValueWithUnitValue(options: {
  ...
  if (options.mode === 'number-or-keyword-select' && options.selectedOptionInputMode === 'numeric') {
    return `${options.inputValue}${options.selectedOptionValue}`;
  }
  return null;
}
...
function resolveSuggestionLabel(options: {
  ...
  if (options.selectedOptionInputMode === 'numeric' && options.selectedOptionValue) {
...
export function ValueWithUnit({
  ...
  const resolvedOptionInputMode =
    resolvedOption?.inputMode ?? (mode === 'keyword-select' ? 'keyword' : 'numeric');
  ...
  const resolvedSuggestionListId = suggestionListId ?? `value-with-unit-${generatedListId}`;
  const activeSuggestionId =
    activeSuggestionIndex >= 0 ? `${resolvedSuggestionListId}-option-${activeSuggestionIndex}` : undefined;

  const showsInput =
    mode === 'number-fixed' ||
    mode === 'number-select' ||
    (mode === 'number-or-keyword-select' && resolvedOptionInputMode === 'numeric');
  const showsFullTrigger =
    mode === 'keyword-select' ||
    (mode === 'number-or-keyword-select' && resolvedOptionInputMode === 'keyword');
  const hasSelectableSuffix = mode === 'number-select' || mode === 'number-or-keyword-select' || mode === 'keyword-select';
  ...
  function handleOptionChange(nextOption: string) {
    ...
    const nextOptionMeta = selectableOptions.find((option) => option.value === nextOption);
    if (mode === 'number-or-keyword-select' && nextOptionMeta?.inputMode === 'keyword') {
      onChange(nextOption);
      return;
    }
  ...
  function handleSuggestionKeyDown(key: string) {
    ...
    if (key === 'ArrowDown') {
      ...
      setActiveSuggestionIndex((current) =>
        current < 0 ? 0 : clampSuggestionIndex(current + 1, suggestionList.length),
      );
      return true;
    }
    ...
    if (key === 'Enter' && suggestionsOpen && activeSuggestionIndex >= 0) {
      handleSuggestionSelection(resolvedSuggestions[activeSuggestionIndex]?.value ?? '');
      return true;
    }
      ...
      >
        {showsInput ? (
          <Input
            ...
            aria-controls={hasSuggestions ? resolvedSuggestionListId : undefined}
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive ? '[background:var(--editor-select-highlight-background)]' : null,
                )}
                onMouseDown={(event) => {
                  event.preventDefault();
                }}
